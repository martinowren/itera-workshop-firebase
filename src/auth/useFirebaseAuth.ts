...
function useFirebaseAuth() {
    ...
    const logout = useCallback(() => {
        return firebase.auth().signOut().then(() => {
            setAuthContext(state => Object.assign({}, state, { user: null, userData: null, isLoggedIn: false }))
        }).catch((error) => {
    ...
    useEffect(() => {
        setAuthContext(state => Object.assign({}, state, {
            login, 
            logout, 
        }));
    }, [login, logout]);
    ...
    useEffect(() => {
        firebase.auth().onAuthStateChanged(user => {
            if (user) {
                ...
                console.error('No valid cookie present, the user needs to log in again.')
            }
        });
    }, [login, logout]);
