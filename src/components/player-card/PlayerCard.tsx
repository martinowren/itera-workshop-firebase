...
export const PlayerCard: FC<PlayerCardProps> = ({
	...
	const playerTurn = currentRound.turns.find(
		(t) => t.player.uid === player.uid
	);
	...
	return (
		<div className="player">
			<div className="player__card-frame">
				{playerTurn && (
					<WhiteCard
						...
						button={
							playerIsCardTsar ? (
								<Button onClick={() => declareWinner(player)}>
									Select winner
								</Button>
							) : null
						}
			...
			<div className="player__username">{player.displayName}</div>
